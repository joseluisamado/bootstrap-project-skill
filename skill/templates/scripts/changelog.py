...
The entry point for cutting a release version. It:

  1. parses the Conventional Commit subjects since the last release tag and
  ...
  2. bumps the ``VERSION`` file (``--level major|minor|patch`` or an explicit
     ``--set X.Y.Z``) and propagates it to per-stack manifests via
     ``sync_version`` (its ``TARGETS`` list),
...
(check → cut → build → commit → tag) and reverts the cut if the build fails.
Normally you invoke it via ``make release``, not directly. The usual flow:

  1. commit your code changes (Conventional Commits: feat:, fix:, refactor!:)
  2. make release            → prompts for LEVEL, runs this script, builds,
                               commits VERSION+CHANGELOG, tags
...
Modes (mutually exclusive):
  ...
  --revert          restore VERSION/CHANGELOG (+ manifests) to HEAD
  --backfill        reseed the whole changelog from every tag (one-off)

Usage:
    ./scripts/changelog.py --level minor          # bump + changelog
    ./scripts/changelog.py                        # prompt for level
    ./scripts/changelog.py --set 2.1.0            # explicit version
    ./scripts/changelog.py --level patch --dry-run
    ./scripts/changelog.py --check                # any unreleased commits?
    ./scripts/changelog.py --revert               # undo an aborted cut
    ./scripts/changelog.py --backfill             # seed from all tags
"""
...
# the single-source-of-truth rules (semver validation, manifest sync) stay in
# one place.
...
# build, ci, test, style) is dropped — those aren't release notes.
...
@dataclass
class Section:
    """Parsed changelog entries grouped by section name."""
...
def unreleased_commits() -> list[str]:
    """Commit subjects since the last release tag."""
...
def _propagate(version: str) -> None:
    """Push the new version into per-stack manifests (sync_version TARGETS)."""
    for path, kind in sync_version.TARGETS:
        sync_version.update_target(path, kind, version)


# Files the cut touches; reverted together if a release build fails. Manifest
# paths come from sync_version's TARGETS so this stays stack-agnostic.
def _release_files() -> list[str]:
    files = ["VERSION", "CHANGELOG.md"]
    files += [str(path.relative_to(ROOT)) for path, _ in sync_version.TARGETS]
    return files


def prompt_level(current: str) -> str:
    """Ask which level to bump, showing the resulting version."""
...
def revert() -> int:
    ...
    Tracked files are checked out from HEAD; an untracked file (e.g. a very
    first CHANGELOG.md) is removed, since "before the cut" means it didn't
    exist. ``git checkout -- <untracked>`` errors, so handle them separately.
    """
    done: list[str] = []
    for f in _release_files():
        if _is_tracked(f):
...
def backfill() -> int:
    ...
    for tag in tags:
        ...
        prev = tag
    body = "\n".join(reversed(blocks))  # newest first
...
def cut(level: str | None, set_to: str | None, dry_run: bool) -> int:
    ...
    print(f"wrote VERSION, CHANGELOG.md{' + manifests' if sync_version.TARGETS else ''}")
    return 0
...
def main() -> int:
    ...
    g.add_argument("--backfill", action="store_true", help="seed changelog from tags")
    g.add_argument("--check", action="store_true", help="exit 0 if commits since tag")
    g.add_argument("--revert", action="store_true", help="restore release files to HEAD")
